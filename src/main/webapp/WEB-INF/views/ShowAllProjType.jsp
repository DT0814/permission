    ...
    <div class="row table-responsive" style="text-align: center;">
        <table class="checkbox-round table table-hover table-responsive table-bordered "
            ...
            <tr class="row">
                ...
                <td>创建时间</td>
                <td></td>
                <td><span class="btn btn-success " v-on:click="createTpye()">创建项目类型</span></td>
            </tr>
            <tr class="row" v-for="(ty,index) in types" v-cloak>
                <td>
                    {{index+1}}
                </td>
                <td>{{ty.name}}</td>
                <td> {{ dateFormat(ty.startdate)}}</td>
                <td>
                    <span class="btn btn-info " v-on:click="showFileNameModal(ty)">查看项目类型文件名</span>
                </td>
                <td>
                    <div class="btn-group" role="group" aria-label="...">
                        <button class="btn btn-primary " v-on:click="updateModal(ty)">修改</button>
                        <button class="btn btn-danger" v-on:click="deleteC(ty)">删除</button>
                    </div>
                </td>
            </tr>
        </table>
